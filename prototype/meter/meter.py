import os, sys, time, glob, Clarens, string
...
#TMB - Added these private objects to hold the jclarens connection information
__serverAddress = 'https://cd-psg2.fnal.gov:8443'
...
def Initialize():
    ...
    #TMB - Added a global jclarens connection object so it can be reused for the lifetime of the server
    global __jclarensConnection
    __jclarensConnection = None

#TMB - Added this method, required by sendUsageXML
##
## encodeXML
##
## Author - Tim Byrne
...
##  to be included in a JClarens call without interfering with the JClarens-specific xml.
##
## param - xmlData:  The xml to encode
## returns - the encoded xml
##
def __encodeXML(xmlData):
    return string.rstrip(xmlData).replace("<", "&lt;").replace(">", "&gt;").replace("'", "&apos;").replace('"', "&aquot;")
...
## Author - Tim Byrne
##
## Connect to the jclarens request processor (xmlrpc) on the given server, sets the module-level object __jclarensConnectio
##  equal to the new connection.  Will not reconnect if __jclarensConnection is already connected.
##
def __connectJClarens():
    global __jclarensConnection

    if __jclarensConnection == None:
        fullPath = __serverAddress + '/jclarens/xmlrpc'
        __jclarensConnection = Clarens.client(fullPath)
        DebugPrint(1, 'Connected to:  ' + fullPath)

#TMB - Added this method jclarens can be disconnected as soon as it is no longer needed
##
## __disconnectJClarens
##
## Author - Tim Byrne
##
## Disconnects the module-level object __jclarensConnection.
##
def __disconnectJClarens():
    global __jclarensConnection

    #if __jclarensConnection != None:
    __jclarensConnection.system.logout()
    DebugPrint(1, 'Disconnected from ' + __serverAddress + '/jclarens/xmlrpc' )
        
#TMB - Added this method to actually send the usage xml to the web service
##
## sendUsageXML
##
## Author - Tim Byrne
...
def __sendUsageXML(meterId, recordXml):
    ...
    # Connect to JClarens, in case we aren't already connected.  If we are already connected, this call will do nothing
    __connectJClarens()

    # Generate a unique Id for this transaction
    transactionId = meterId + time.strftime('%S')
    DebugPrint(1, 'TransactionId:  ' + transactionId)

    # Currently, the recordXml is in a list format, with each item being a line of xml.  JClarens requires the xml to be sent
    #  as a string.  This little logic here turns the xml list into a single xml string.
    usageXmlString = ""
    for line in recordXml:
        usageXmlString = usageXmlString + line
    DebugPrint(3, 'UsageXml:  ' + usageXmlString)


    # Invoke the collectUsageXML web service
    response = __jclarensConnection.Collector.collectUsageXML(transactionId, __encodeXML(usageXmlString))
    DebugPrint(1, 'Resonse:  ' + response)
...
def Send(record):
    ...
    #TMB - Added this line to send the record's XML to the collector web service
    #TMB - TODO:  How will we determine the name of this meter, so the collector can identify it?
    response = __sendUsageXML('Metername', record.XmlData)

    #TMB - TODO:  Parse the response to determine if the file was sent successfully or not.  We have not yet decided on appropriate return codes
    # So this functionality cannot be coded yet
    ...
    #TMB - When we are done sending outstanding records, we need to then disconnect JClarens.  Whether or not that actually occurs here
    # depends on how outstanding records end up being processed
    __disconnectJClarens()
