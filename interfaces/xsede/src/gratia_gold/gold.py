...
def drop_privs(cp):
    ...
    try:
        ...
        if oe.errno != errno.EPERM:
            raise Exception("Unable to drop privileges to %s" % gold_user)
...
def call_gcharge(job, gcharge_machine):
    '''
    ...
    if pid == 0:
        ...
        try:
            ...
            raise Exception("os.execvp failed; error code is: "+str(execvpstatus))
    status = 0
    pid2 = 0
    while pid != pid2:
        pid2, status = os.wait()
