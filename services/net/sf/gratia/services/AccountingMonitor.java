				...
				while (iter.hasMoreElements())
						{
								...
								Logging.info("MonitorService: Monitoring Server: " + key + " Current DBID: " + value);
						}
				//
								...
								while(iter.hasMoreElements())
										{
												...
												try
														{
																Post post = new Post(to,"remoteRequest",from,to,"" + value.longValue());
																post.add("from",from);
																post.add("to",to);
																post.add("rmi",externalrmi);
																post.send();
																Logging.info("AccountingMonitor: Sent Request To: " + to + " DBID: " + value);
														}
												catch (Exception e)
														{
																Logging.info("AccountingMonitor: Unable To Send Request To: " + to);
