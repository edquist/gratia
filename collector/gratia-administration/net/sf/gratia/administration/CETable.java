...
public class CETable extends HttpServlet 
{
		XP xp = new XP();
		//
		// database related
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
		...
		public void setup()
		{
				html = xp.get(request.getRealPath("/") + "cetable.html");
				m = p.matcher(html);
				while (m.find())
						{
								String temp = m.group();
		...
		{
				int index = 0;
				String command = "select * from CETable order by facility_name";
		...
		{
				String command = 
						"update CETable set" + cr +
						" facility_name = " + dq + (String) request.getParameter("cename:" + index) + dq + cr +
						" where facility_id = " + request.getParameter("dbid:" + index);
				try
						{
		...
		{
				String command = 
						"insert into CETable (facility_name) values(" + 
