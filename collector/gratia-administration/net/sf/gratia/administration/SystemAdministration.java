...
public class SystemAdministration extends HttpServlet {
   ...
   public void process() {
      ...
      if (proxy != null ) {
         try {
            ...
            if (flag) {
               ...
               queuemanagerStatus = proxy.queueManagerStatus();
            }
         }
         catch (Exception e) {
      ...
      html = html.replaceAll("#housekeepingstatus#",
                             ...
                             housekeepingStatus +
                             "</strong></font>");
      if (queuemanagerStatus.equals("UpToDate")) {
         color = "green";
      } else if (queuemanagerStatus.equals("OutOfDate")) {
         color = "red";
      } else {
         color = "fuchsia";
      }
      html = html.replaceAll("#queuemanagerstatus#",
                             "<font color=\"" +
                             color +
                             "\"><strong>" +
                             queuemanagerStatus +
                             "</strong></font>");
      html = html.replaceAll("#queuemanagercomment#","<a href=\"systemadministration.html?action=refreshStatus\"><strong>Refresh</strong></a> the collector status by recalculating the queues sizes directly from the files on the disk queue (Strongly recommended if you added or removed input files manually from the queue.)");
   }
